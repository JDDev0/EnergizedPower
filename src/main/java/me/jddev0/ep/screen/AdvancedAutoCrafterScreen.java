...
public class AdvancedAutoCrafterScreen extends ConfigurableUpgradableEnergyStorageContainerScreen<AdvancedAutoCrafterMenu> {
    ...
    private void renderCheckboxes(GuiGraphicsExtractor drawContext, int x, int y, int mouseX, int mouseY) {
        ...
        if(menu.isSecondaryExtractMode()) {
            ...
            drawContext.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 158, y + 38, 22, 139, 11, 11, 256, 256);
        }else {
            //Extract mode checkbox [1]

            drawContext.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 158, y + 38, 11, 139, 11, 11, 256, 256);
        }

        if(menu.isAllowOutputOverflow()) {
            //Allow Output Overflow checkbox

            drawContext.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 158, y + 59, 0, 139, 11, 11, 256, 256);
        }

        drawContext.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 96, y + 16, 11 + 11 * menu.getRecipeIndex(), 139, 11, 11, 256, 256);
    }
