        ...
        if(mouseButton == 0) {
            ...
            if(!handler.isInUpgradeModuleView()) {
                if(isPointWithinBounds(158, 16, 11, 11, mouseX, mouseY)) {
                    ...
                    ModMessages.sendClientPacketToServer(
                            new CycleAdvancedAutoCrafterRecipeOutputC2SPacket(handler.getBlockEntity().getPos()));
                    clicked = true;
                }else if(isPointWithinBounds(96, 16, 12, 12, mouseX, mouseY)) {
                    //Set recipe index

                    ModMessages.sendClientPacketToServer(
                            new SetAdvancedAutoCrafterRecipeIndexC2SPacket(handler.getBlockEntity().getPos(), handler.getRecipeIndex() + 1));
                    clicked = true;
                }
            }
