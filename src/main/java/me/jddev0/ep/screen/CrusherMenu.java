...
public class CrusherMenu extends UpgradableEnergyStorageMenu<CrusherBlockEntity>
    ...
    public CrusherMenu(int id, Inventory inv, BlockPos pos) {
        this(id, inv.player.level().getBlockEntity(pos), inv, new SimpleContainer(2) {
            @Override
            public boolean canPlaceItem(int slot, ItemStack stack) {
                return switch(slot) {
                    case 0 -> RecipeUtils.isIngredientOfAny(inv.player.level(), CrusherRecipe.Type.INSTANCE, stack);
                    case 1 -> false;
    ...
    public CrusherMenu(int id, BlockEntity blockEntity, Inventory playerInventory, Container inv,
                       UpgradeModuleInventory upgradeModuleInventory, ContainerData data) {
        super(
                EPMenuTypes.CRUSHER_MENU, id,
