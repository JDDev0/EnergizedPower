...
public class AdvancedPoweredFurnaceMenu extends UpgradableEnergyStorageMenu<AdvancedPoweredFurnaceBlockEntity>
    ...
    public AdvancedPoweredFurnaceMenu(int id, Inventory inv, BlockPos pos) {
        this(id, inv, (AdvancedPoweredFurnaceBlockEntity)inv.player.level().getBlockEntity(pos));
    }
    private AdvancedPoweredFurnaceMenu(int id, Inventory inv, AdvancedPoweredFurnaceBlockEntity blockEntity) {
        this(id, blockEntity, inv, new SimpleContainer(6) {
            @Override
            public boolean canPlaceItem(int slot, ItemStack stack) {
                return switch(slot) {
                    case 0, 1, 2 -> RecipeUtils.isIngredientOfAny(inv.player.level(),
    ...
    public AdvancedPoweredFurnaceMenu(int id, BlockEntity blockEntity, Inventory playerInventory, Container inv,
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if(index < 4 * 9) {
            ...
            if(!moveItemStackTo(sourceItem, 4 * 9 + 6, 4 * 9 + 6 + 6, false) &&
