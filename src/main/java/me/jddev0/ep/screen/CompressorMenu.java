...
public class CompressorMenu extends UpgradableEnergyStorageMenu<CompressorBlockEntity>
    ...
    public CompressorMenu(int id, Inventory inv, BlockPos pos) {
        this(id, inv.player.level().getBlockEntity(pos), inv, new SimpleContainer(2) {
            @Override
            public boolean canPlaceItem(int slot, ItemStack stack) {
                return switch(slot) {
                    case 0 -> RecipeUtils.isIngredientOfAny(inv.player.level(), CompressorRecipe.Type.INSTANCE, stack);
                    case 1 -> false;
    ...
    public CompressorMenu(int id, BlockEntity blockEntity, Inventory playerInventory, Container inv,
                          UpgradeModuleInventory upgradeModuleInventory, ContainerData data) {
        super(
                EPMenuTypes.COMPRESSOR_MENU, id,
