...
public class XPStorageScreen extends EnergizedPowerBaseContainerScreen<XPStorageMenu> {
    ...
    private void renderXPBar(GuiGraphicsExtractor guiGraphics, int x, int y) {
        ...
        guiGraphics.text(font, component, (int)(x + (176 - componentWidth) * .5f), y + 36, 0xFF000000, false);
        ...
        if(xpBarProgress > 0)
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 7, y + 49, 0, 251, xpBarProgress, 5, 256, 256);
    }
    ...
    private void renderButtons(GuiGraphicsExtractor guiGraphics, int x, int y, int mouseX, int mouseY) {
        for(int i = 0;i < 2;i++) {
            ...
            if(isHovering(45, 16 + yPosDelta, 13, 14, mouseX, mouseY)) {
                ...
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 45, y + 16 + yPosDelta, 92, 193 + vPosDelta, 13, 14, 256, 256);
            }else if(isHovering(60, 16 + yPosDelta, 18, 14, mouseX, mouseY)) {
                //+/-10 buttons

                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 60, y + 16 + yPosDelta, 105, 193 + vPosDelta, 18, 14, 256, 256);
            }else if(isHovering(80, 16 + yPosDelta, 24, 14, mouseX, mouseY)) {
                //+/-100 buttons

                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 80, y + 16 + yPosDelta, 123, 193 + vPosDelta, 24, 14, 256, 256);
            }else if(isHovering(106, 16 + yPosDelta, 26, 14, mouseX, mouseY)) {
                //+/-MAX buttons

                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 106, y + 16 + yPosDelta, 147, 193 + vPosDelta, 26, 14, 256, 256);
            }
        }
    }
}
