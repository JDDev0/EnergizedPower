...
public class FiltrationPlantScreen extends SelectableRecipeMachineContainerScreen<FiltrationPlantRecipe, FiltrationPlantMenu> {
    ...
    @Override
    protected ItemStack getRecipeIcon(RecipeEntry<FiltrationPlantRecipe> currentRecipe) {
        Identifier icon = currentRecipe.value().getIcon();
        return new ItemStack(Registries.ITEM.get(icon));
    }

    @Override
    protected void renderCurrentRecipeTooltip(DrawContext guiGraphics, int mouseX, int mouseY, RecipeEntry<FiltrationPlantRecipe> currentRecipe) {
    ...
    private void renderProgressArrows(DrawContext drawContext, int x, int y) {
        if(handler.isCraftingActive()) {
            for(int i = 0;i < 2;i++) {
                drawContext.drawTexture(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 67, y + 34 + 27*i, 0, 108, handler.getScaledProgressArrowSize(), 9, 256, 256);
            }
        }
    }
