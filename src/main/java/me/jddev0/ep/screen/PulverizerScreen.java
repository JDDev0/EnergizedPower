...
public class PulverizerScreen
        extends ConfigurableIOUpgradableEnergyStorageContainerScreen<PulverizerMenu> {
    public PulverizerScreen(PulverizerMenu menu, Inventory inventory, Component component) {
        super(menu, inventory, component,
                "tooltip.energizedpower.recipe.energy_required_to_finish.txt",
                EPAPI.id("textures/gui/container/pulverizer.png"),
    ...
    private void renderProgressArrow(GuiGraphicsExtractor drawContext, int x, int y) {
        if(menu.isCraftingActive())
            drawContext.blit(RenderPipelines.GUI_TEXTURED, MACHINE_SPRITES_TEXTURE, x + 68, y + 34, 0, 58, menu.getScaledProgressArrowSize(), 17, 256, 256);
    }
}
