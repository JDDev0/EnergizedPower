...
public class WeatherControllerScreen extends UpgradableEnergyStorageContainerScreen<WeatherControllerMenu> {
    ...
    private void renderButtons(GuiGraphics drawContext, int x, int y, int mouseX, int mouseY) {
        ...
        //Weather clear button
        if(isHovering(52, 34, 18, 18, mouseX, mouseY)) {
            drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 52, y + 34, 56, 193, 18, 18);
        }else if(selectedWeatherType == 0) {
            drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 52, y + 34, 74, 193, 18, 18);
        }

        //Weather rain button
        if(isHovering(88, 34, 18, 18, mouseX, mouseY)) {
            drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 88, y + 34, 56, 211, 18, 18);
        }else if(selectedWeatherType == 1) {
            drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 88, y + 34, 74, 211, 18, 18);
        }

        //Weather thunder button
        if(isHovering(124, 34, 18, 18, mouseX, mouseY)) {
            drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 124, y + 34, 56, 229, 18, 18);
        }else if(selectedWeatherType == 2) {
            drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 124, y + 34, 74, 229, 18, 18);
        }
    }
    ...
    private void renderInfoText(GuiGraphics drawContext, int x, int y) {
        ...
        drawContext.drawString(font, component, (int)(x + 34 + (126 - componentWidth) * .5f), y + 58, 0, false);
    }
