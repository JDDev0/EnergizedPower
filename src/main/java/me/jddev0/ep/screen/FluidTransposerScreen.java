...
public class FluidTransposerScreen
    ...
    private void renderButtons(GuiGraphics drawContext, int x, int y, int mouseX, int mouseY) {
        if(isHovering(114, 47, 20, 20, mouseX, mouseY))
            drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 114, y + 47, 0, 211, 20, 20);
    ...
    private void renderProgressArrow(GuiGraphics drawContext, int x, int y) {
        ...
        drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 114, y + 19, 52, arrowPosY, 20, 14);

        if(menu.isCraftingActive()) {
            if(menu.getMode() == FluidTransposerBlockEntity.Mode.EMPTYING)
                drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 114, y + 19, 72, arrowPosY, menu.getScaledProgressArrowSize(), 14);
            else
                drawContext.blit(MACHINE_SPRITES_TEXTURE, x + 134 - menu.getScaledProgressArrowSize(), y + 19,
