...
public class FluidTransposerScreen
    ...
    private void renderFluidMeterOverlay(DrawContext drawContext, int x, int y) {
        drawContext.drawTexture(MACHINE_SPRITES_TEXTURE, x + 152, y + 17, 16, 0, 16, 52);
    }

    private void renderButtons(DrawContext drawContext, int x, int y, int mouseX, int mouseY) {
        if(isPointWithinBounds(114, 47, 20, 20, mouseX, mouseY))
            drawContext.drawTexture(MACHINE_SPRITES_TEXTURE, x + 114, y + 47, 0, 211, 20, 20);
    ...
    private void renderProgressArrow(DrawContext drawContext, int x, int y) {
        ...
        drawContext.drawTexture(MACHINE_SPRITES_TEXTURE, x + 114, y + 19, 52, arrowPosY, 20, 14);

        if(handler.isCraftingActive()) {
            if(handler.getMode() == FluidTransposerBlockEntity.Mode.EMPTYING)
                drawContext.drawTexture(MACHINE_SPRITES_TEXTURE, x + 114, y + 19, 72, arrowPosY, handler.getScaledProgressArrowSize(), 14);
            else
                drawContext.drawTexture(MACHINE_SPRITES_TEXTURE, x + 134 - handler.getScaledProgressArrowSize(), y + 19,
                        92 - handler.getScaledProgressArrowSize(), arrowPosY, handler.getScaledProgressArrowSize(), 14);
        }
    }
