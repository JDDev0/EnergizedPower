...
public class EnergizerMenu extends AbstractEnergizedPowerScreenHandler
    ...
    public EnergizerMenu(int id, PlayerInventory inv, PacketByteBuf buf) {
        this(id, inv.player.getWorld().getBlockEntity(buf.readBlockPos()), inv, new SimpleInventory(2) {
            ...
            public boolean isValid(int slot, ItemStack stack) {
                return switch(slot) {
                    case 0 -> RecipeUtils.isIngredientOfAny(inv.player.getWorld(), EnergizerRecipe.Type.INSTANCE, stack);
                    case 1 -> false;
    ...
    public ItemStack quickMove(PlayerEntity player, int index) {
        ...
        if(index < 4 * 9) {
            ...
            if(!insertMaxCount1Item(sourceItem, 4 * 9 + 2, 4 * 9 + 2 + 1, false) &&
    ...
    public boolean canUse(PlayerEntity player) {
        return canUse(ScreenHandlerContext.create(level, blockEntity.getPos()), player, ModBlocks.ENERGIZER);
    }
