...
public class StoneSolidifierScreen extends SelectableRecipeMachineContainerScreen<StoneSolidifierRecipe, StoneSolidifierMenu> {
    ...
    @Override
    protected ItemStack getRecipeIcon(RecipeEntry<StoneSolidifierRecipe> currentRecipe) {
        return currentRecipe.value().getOutput();
    }

    @Override
    protected void renderCurrentRecipeTooltip(DrawContext drawContext, int mouseX, int mouseY, RecipeEntry<StoneSolidifierRecipe> currentRecipe) {
    ...
    private void renderProgressArrows(DrawContext drawContext, int x, int y) {
        if(handler.isCraftingActive()) {
            drawContext.drawTexture(RenderLayer::getGuiTextured, TEXTURE, x + 69, y + 45, 176, 106, handler.getScaledProgressArrowSize(), 14, 256, 256);
            drawContext.drawTexture(RenderLayer::getGuiTextured, TEXTURE, x + 143 - handler.getScaledProgressArrowSize(), y + 45,
                    196 - handler.getScaledProgressArrowSize(), 120, handler.getScaledProgressArrowSize(), 14, 256, 256);
        }
    }
