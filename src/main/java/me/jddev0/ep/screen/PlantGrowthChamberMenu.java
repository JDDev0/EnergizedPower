    ...
    public PlantGrowthChamberMenu(int id, Inventory inv, BlockPos pos) {
        this(id, inv.player.level().getBlockEntity(pos), inv, new SimpleContainer(7) {
            ...
            public boolean canPlaceItem(int slot, ItemStack stack) {
                return switch(slot) {
                    case 0 -> RecipeUtils.isIngredientOfAny(inv.player.level(), PlantGrowthChamberRecipe.Type.INSTANCE, stack);
                    case 1 -> RecipeUtils.isIngredientOfAny(inv.player.level(), PlantGrowthChamberFertilizerRecipe.Type.INSTANCE, stack);
                    case 6 -> RecipeUtils.isIngredientOfAny(inv.player.level(), PlantGrowthChamberSoilRecipe.Type.INSTANCE, stack);
                    case 2, 3, 4, 5 -> false;
