...
public class AutoStonecutterScreen
    ...
    private void renderCurrentRecipeOutput(DrawContext drawContext, int x, int y) {
        ...
        ItemStack itemStackIcon = currentRecipe.getOutput(handler.getBlockEntity().getWorld().getRegistryManager());
        if(!itemStackIcon.isEmpty()) {
            drawContext.getMatrices().push();
            drawContext.getMatrices().translate(0.f, 0.f, 100.f);

            drawContext.drawItem(itemStackIcon, x + 80, y + 17, 80 + 17 * this.backgroundWidth);
