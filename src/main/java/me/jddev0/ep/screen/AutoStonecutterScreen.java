...
public class AutoStonecutterScreen
    ...
    private void renderCurrentRecipeOutput(DrawContext drawContext, int x, int y) {
        RecipeEntry<StonecuttingRecipe> currentRecipe = handler.getCurrentRecipe();
        if(currentRecipe == null)
            return;

        ItemStack itemStackIcon = currentRecipe.value().getResult(handler.getBlockEntity().getWorld().getRegistryManager());
