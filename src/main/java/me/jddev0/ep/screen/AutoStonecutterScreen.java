...
public class AutoStonecutterScreen
    ...
    protected ItemStack getRecipeIcon(StonecuttingRecipe currentRecipe) {
        return currentRecipe.getOutput(handler.getBlockEntity().getWorld().getRegistryManager());
    }

    @Override
    protected void renderCurrentRecipeTooltip(DrawContext drawContext, int mouseX, int mouseY, StonecuttingRecipe currentRecipe) {
        ItemStack output = currentRecipe.getOutput(handler.getBlockEntity().getWorld().getRegistryManager());
        if(!output.isEmpty()) {
