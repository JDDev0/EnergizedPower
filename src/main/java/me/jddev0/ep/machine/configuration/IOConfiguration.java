...
public class IOConfiguration {
    ...
    public IntArrayTag serialize() {
        return new IntArrayTag(Arrays.copyOf(slotGroupIds, 6));
    }

    public boolean deserialize(IntArrayTag nbt) {
        int[] nbtInts = nbt.getAsIntArray();
        if(nbtInts.length != 6)
            return false;

        System.arraycopy(nbtInts, 0, slotGroupIds, 0, 6);

        return true;
    }

    public InputOutputItemHandler createSidedItemHandlerFor(
            @NotNull List<@NotNull SlotGroup> slotGroups, @NotNull SlottedStorage<ItemVariant> handler,
    ...
    public InputOutputFluidStorage createSidedFluidHandlerFor(
            @NotNull List<@NotNull SlotGroup> slotGroups, @NotNull SlottedStorage<FluidVariant> handler,
            @NotNull Direction facing, @NotNull Direction side
    ) {
        int slotGroupId = getSlotGroupId(RelativeDirection.resolve(facing, side));
        if(slotGroupId != -1 && slotGroupId >= slotGroups.size())
