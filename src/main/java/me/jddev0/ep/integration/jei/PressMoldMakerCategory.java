...
public class PressMoldMakerCategory implements IRecipeCategory<PressMoldMakerRecipe> {
    ...
    @Override
    public RecipeType<PressMoldMakerRecipe> getRecipeType() {
        return TYPE;
    }
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder iRecipeLayoutBuilder, PressMoldMakerRecipe recipe, IFocusGroup iFocusGroup) {
        iRecipeLayoutBuilder.addSlot(RecipeIngredientRole.INPUT, 1, 5).addItemStack(new ItemStack(Items.CLAY_BALL, recipe.getClayCount()));

        iRecipeLayoutBuilder.addSlot(RecipeIngredientRole.OUTPUT, 64, 5).addItemStack(recipe.getOutputItem());
    }
}
