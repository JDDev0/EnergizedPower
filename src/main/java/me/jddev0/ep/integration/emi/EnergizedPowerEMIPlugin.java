...
public class EnergizedPowerEMIPlugin implements EmiPlugin {
    ...
    private void registerRecipes(EmiRegistry registry) {
        ...
        for(MetalPressRecipe recipe:recipeManager.listAllOfType(MetalPressRecipe.Type.INSTANCE))
            registry.addRecipe(new MetalPressEMIRecipe(recipe));

        for(AssemblingMachineRecipe recipe:recipeManager.listAllOfType(AssemblingMachineRecipe.Type.INSTANCE))
        ...
        for(CrystalGrowthChamberRecipe recipe:recipeManager.listAllOfType(CrystalGrowthChamberRecipe.Type.INSTANCE))
            registry.addRecipe(new CrystalGrowthChamberEMIRecipe(recipe));

        for(PressMoldMakerRecipe recipe:recipeManager.listAllOfType(PressMoldMakerRecipe.Type.INSTANCE))
        ...
        for(FiltrationPlantRecipe recipe:recipeManager.listAllOfType(FiltrationPlantRecipe.Type.INSTANCE))
            registry.addRecipe(new FiltrationPlantEMIRecipe(recipe));

        for(FluidTransposerRecipe recipe:recipeManager.listAllOfType(FluidTransposerRecipe.Type.INSTANCE))
        ...
        registry.addRecipe(new DispenserEMIRecipe(new DispenserEMIRecipe.DispenserRecipe(
                ...
                Ingredient.fromTag(ConventionalItemTags.SHEARS), Ingredient.fromTag(ItemTags.WOOL),
    ...
    private void registerRecipeHandlers(EmiRegistry registry) {
        ...
        registry.addRecipeHandler(ModMenuTypes.AUTO_PRESS_MOLD_MAKER_MENU,
                new SelectableRecipeMachineRecipeHandler<>(PressMoldMakerEMIRecipe.CATEGORY));

        registry.addRecipeHandler(ModMenuTypes.AUTO_STONECUTTER_MENU,
                new SelectableRecipeMachineRecipeHandler<>(VanillaEmiRecipeCategories.STONECUTTING));

        registry.addRecipeHandler(ModMenuTypes.STONE_SOLIDIFIER_MENU,
                new SelectableRecipeMachineRecipeHandler<>(StoneSolidifierEMIRecipe.CATEGORY));

        registry.addRecipeHandler(ModMenuTypes.FILTRATION_PLANT_MENU,
