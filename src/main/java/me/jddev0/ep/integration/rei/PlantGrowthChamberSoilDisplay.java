...
public record PlantGrowthChamberSoilDisplay(RecipeHolder<PlantGrowthChamberSoilRecipe> recipe) implements Display {
    ...
    public static final DisplaySerializer<? extends PlantGrowthChamberSoilDisplay> SERIALIZER = DisplaySerializer.of(
            RecordCodecBuilder.mapCodec((instance) -> {
                return instance.group(Identifier.CODEC.fieldOf("recipeId").forGetter(display -> {
                    return display.recipe.id().identifier();
                }), EPRecipes.PLANT_GROWTH_CHAMBER_SOIL_SERIALIZER.get().codec().fieldOf("ingredient").forGetter(display -> {
                    return display.recipe.value();
                })).apply(instance, (recipeId, recipe) -> new PlantGrowthChamberSoilDisplay(new RecipeHolder<>(
            ...
            StreamCodec.composite(
                    ...
                    EPRecipes.PLANT_GROWTH_CHAMBER_SOIL_SERIALIZER.get().streamCodec(),
                    display -> display.recipe.value(),
                    (recipeId, recipe) -> new PlantGrowthChamberSoilDisplay(new RecipeHolder<>(
                            ResourceKey.create(Registries.RECIPE, recipeId), recipe
                    ))
            )
    );
    ...
    @Override
    public DisplaySerializer<? extends PlantGrowthChamberSoilDisplay> getSerializer() {
        return SERIALIZER;
    }
}