...
public record CrusherDisplay(RecipeHolder<CrusherRecipe> recipe) implements Display {
    ...
    public static final DisplaySerializer<? extends CrusherDisplay> SERIALIZER = DisplaySerializer.of(
            RecordCodecBuilder.mapCodec((instance) -> {
                ...
                }), EPRecipes.CRUSHER_SERIALIZER.get().codec().fieldOf("ingredient").forGetter(display -> {
                    return display.recipe.value();
                })).apply(instance, (recipeId, recipe) -> new CrusherDisplay(new RecipeHolder<>(
            ...
            StreamCodec.composite(
                    ...
                    EPRecipes.CRUSHER_SERIALIZER.get().streamCodec(),
                    display -> display.recipe.value(),
                    (recipeId, recipe) -> new CrusherDisplay(new RecipeHolder<>(
                            ResourceKey.create(Registries.RECIPE, recipeId), recipe
                    ))
            )
    );
