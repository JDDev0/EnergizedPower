...
public class ModLanguageProvider extends ExtensionLanguageProvider {
    ...
    public ModLanguageProvider(FabricDataOutput output, CompletableFuture<HolderLookup.Provider> lookupProvider,
                               String locale, String sourceLangFilePath) {
    ...
    protected final void addExtendedTranslations(HolderLookup.Provider registryLookup, TranslationBuilder translationBuilder) {
        Map<String, Integer> upgradeModules = new HashMap<>();
        {
            ...
            upgradeModules.put("moon_light", 5);
            upgradeModules.put("item_ejector", 8);
            upgradeModules.put("item_pulling", 8);
            upgradeModules.put("xp_extraction", 8);
        }
        ...
        for(String templateTranslation:templateTranslations.keySet()) {
            if(templateTranslation.startsWith("_template.epblock.")) {
                ...
                translationBuilder.add("block.energizedpower." + keySuffix, value);
                translationBuilder.add("item.energizedpower." + keySuffix, value);
                ...
                translationBuilder.add("block.energizedpower." + keySuffix, value);
                translationBuilder.add("item.energizedpower." + keySuffix, value);
                translationBuilder.add("container.energizedpower." + keySuffix, value);
            }
        }
    }
}
