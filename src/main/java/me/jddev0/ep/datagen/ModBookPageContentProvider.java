...
public class ModBookPageContentProvider extends PageContentProvider {
    ...
    private void registerEnergyBlocksChapterEnergyConsumptionSubChapter() {
        ...
        {
            Map<Integer, Identifier> changePageIntToId = new HashMap<>();
            addSimplePage(pageId("fluid_pump"), Text.empty().append(
                    Text.translatable("book.energizedpower.page.fluid_pump").
                            formatted(Formatting.BLACK, Formatting.BOLD).fillStyle(UNIFORM)
            ), EPBlocks.FLUID_PUMP, changePageIntToId);
        }
    ...
    private MutableText tableOfContentEntryComponent(String chapterTitle, PageContent chapterPage,
        ...
        return Text.translatable(chapterTitle).formatted(Formatting.DARK_GREEN, Formatting.UNDERLINE).
                fillStyle(Style.EMPTY.
                        ...
                        withHoverEvent(new HoverEvent.ShowText(Text.translatable("book.energizedpower.tooltip.page"))));
    }
    private MutableText backToTableOfContentComponent(String tableOfContentPageId, Map<Integer, Identifier> changePageIntToId) {
        int pageNum = changePageIntToId.keySet().stream().max(Integer::compareTo).orElse(0) + 1;
        changePageIntToId.put(pageNum, Identifier.of(EPAPI.MOD_ID, tableOfContentPageId));
