...
public class AdvancedUnchargerBlock extends BlockWithEntity {
    public static final MapCodec<AdvancedUnchargerBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> {
        ...
        })).apply(instance, AdvancedUnchargerBlock::new);
    });

    public static final BooleanProperty POWERED = Properties.POWERED;

    public AdvancedUnchargerBlock(FabricBlockSettings props) {
        super(props);
    ...
    public BlockEntity createBlockEntity(BlockPos blockPos, BlockState state) {
        return new AdvancedUnchargerBlockEntity(blockPos, state);
    }
    ...
    public int getComparatorOutput(BlockState state, World level, BlockPos blockPos) {
        ...
        if(!(blockEntity instanceof AdvancedUnchargerBlockEntity advancedUnchargerBlockEntity))
            return super.getComparatorOutput(state, level, blockPos);

        return advancedUnchargerBlockEntity.getRedstoneOutput();
    }
    ...
    public void onStateReplaced(BlockState state, World level, BlockPos blockPos, BlockState newState, boolean isMoving) {
        ...
        ((AdvancedUnchargerBlockEntity)blockEntity).drops(level, blockPos);
    ...
    public ActionResult onUse(BlockState state, World level, BlockPos blockPos, PlayerEntity player, Hand handItem, BlockHitResult hit) {
        ...
        player.openHandledScreen((AdvancedUnchargerBlockEntity)blockEntity);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World level, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.ADVANCED_UNCHARGER_ENTITY, AdvancedUnchargerBlockEntity::tick);
    }
}
