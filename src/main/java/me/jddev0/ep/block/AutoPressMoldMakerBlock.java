...
public class AutoPressMoldMakerBlock extends BlockWithEntity {
    public static final MapCodec<AutoPressMoldMakerBlock> CODEC = createCodec(AutoPressMoldMakerBlock::new);

    public static final BooleanProperty POWERED = Properties.POWERED;

    public AutoPressMoldMakerBlock(AbstractBlock.Settings props) {
        super(props);
    ...
    public BlockEntity createBlockEntity(BlockPos blockPos, BlockState state) {
        return new AutoPressMoldMakerBlockEntity(blockPos, state);
    }
    ...
    public int getComparatorOutput(BlockState state, World level, BlockPos blockPos) {
        ...
        if(!(blockEntity instanceof AutoPressMoldMakerBlockEntity autoPressMoldMakerBlockEntity))
            return super.getComparatorOutput(state, level, blockPos);

        return autoPressMoldMakerBlockEntity.getRedstoneOutput();
    }
    ...
    public void onStateReplaced(BlockState state, World level, BlockPos blockPos, BlockState newState, boolean isMoving) {
        ...
        BlockEntity blockEntity = level.getBlockEntity(blockPos);
        if(!(blockEntity instanceof AutoPressMoldMakerBlockEntity))
            return;

        ((AutoPressMoldMakerBlockEntity)blockEntity).drops(level, blockPos);
    ...
    public ActionResult onUse(BlockState state, World level, BlockPos blockPos, PlayerEntity player, BlockHitResult hit) {
        ...
        player.openHandledScreen((AutoPressMoldMakerBlockEntity)blockEntity);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World level, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.AUTO_PRESS_MOLD_MAKER_ENTITY, AutoPressMoldMakerBlockEntity::tick);
    }
}
