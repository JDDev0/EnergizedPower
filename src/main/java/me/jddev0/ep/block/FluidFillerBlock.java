...
public class FluidFillerBlock extends BlockWithEntity {
    public static final MapCodec<FluidFillerBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> {
        ...
        })).apply(instance, FluidFillerBlock::new);
    });

    public static final BooleanProperty POWERED = Properties.POWERED;

    public FluidFillerBlock(FabricBlockSettings props) {
        super(props);
    ...
    public BlockEntity createBlockEntity(BlockPos blockPos, BlockState state) {
        return new FluidFillerBlockEntity(blockPos, state);
    }
    ...
    public int getComparatorOutput(BlockState state, World level, BlockPos blockPos) {
        ...
        if(!(blockEntity instanceof FluidFillerBlockEntity fluidFillerBlockEntity))
            return super.getComparatorOutput(state, level, blockPos);

        return fluidFillerBlockEntity.getRedstoneOutput();
    }
    ...
    public void onStateReplaced(BlockState state, World level, BlockPos blockPos, BlockState newState, boolean isMoving) {
        ...
        BlockEntity blockEntity = level.getBlockEntity(blockPos);
        if(!(blockEntity instanceof FluidFillerBlockEntity))
            return;

        ((FluidFillerBlockEntity)blockEntity).drops(level, blockPos);
    ...
    public ActionResult onUse(BlockState state, World level, BlockPos blockPos, PlayerEntity player, Hand handItem, BlockHitResult hit) {
        ...
        player.openHandledScreen((FluidFillerBlockEntity)blockEntity);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World level, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.FLUID_FILLER_ENTITY, FluidFillerBlockEntity::tick);
    }
}
