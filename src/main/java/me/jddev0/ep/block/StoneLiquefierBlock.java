...
public class StoneLiquefierBlock extends BlockWithEntity {
    public static final MapCodec<StoneLiquefierBlock> CODEC = createCodec(StoneLiquefierBlock::new);

    public static final BooleanProperty POWERED = Properties.POWERED;
    public static final EnumProperty<Direction> FACING = Properties.HORIZONTAL_FACING;

    protected StoneLiquefierBlock(AbstractBlock.Settings props) {
        super(props);
    ...
    public BlockEntity createBlockEntity(BlockPos blockPos, BlockState state) {
        return new StoneLiquefierBlockEntity(blockPos, state);
    }
    ...
    public int getComparatorOutput(BlockState state, World level, BlockPos blockPos) {
        ...
        if(!(blockEntity instanceof StoneLiquefierBlockEntity StoneLiquefierBlockEntity))
            return super.getComparatorOutput(state, level, blockPos);

        return StoneLiquefierBlockEntity.getRedstoneOutput();
    }
    ...
    public void onStateReplaced(BlockState state, World level, BlockPos blockPos, BlockState newState, boolean isMoving) {
        ...
        BlockEntity blockEntity = level.getBlockEntity(blockPos);
        if(!(blockEntity instanceof StoneLiquefierBlockEntity))
            return;

        ((StoneLiquefierBlockEntity)blockEntity).drops(level, blockPos);
    ...
    public ActionResult onUse(BlockState state, World level, BlockPos blockPos, PlayerEntity player, BlockHitResult hit) {
        ...
        player.openHandledScreen((StoneLiquefierBlockEntity)blockEntity);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World level, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, EPBlockEntities.STONE_LIQUEFIER_ENTITY, StoneLiquefierBlockEntity::tick);
    }
}
