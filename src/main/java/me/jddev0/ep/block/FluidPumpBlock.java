...
public class FluidPumpBlock extends BlockWithEntity {
    public static final BooleanProperty POWERED = Properties.POWERED;

    public FluidPumpBlock(AbstractBlock.Settings props) {
        super(props);
    ...
    public BlockEntity createBlockEntity(BlockPos blockPos, BlockState state) {
        return new FluidPumpBlockEntity(blockPos, state);
    }
    ...
    public int getComparatorOutput(BlockState state, World level, BlockPos blockPos) {
        ...
        if(!(blockEntity instanceof FluidPumpBlockEntity fluidPumpBlockEntity))
            return super.getComparatorOutput(state, level, blockPos);

        return fluidPumpBlockEntity.getRedstoneOutput();
    }
    ...
    public ActionResult onUse(BlockState state, World level, BlockPos blockPos, PlayerEntity player, Hand handItem, BlockHitResult hit) {
        ...
        player.openHandledScreen((FluidPumpBlockEntity)blockEntity);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World level, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.FLUID_PUMP_ENTITY, FluidPumpBlockEntity::tick);
    }
}
