...
public class AdvancedCrusherBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory<BlockPos>, EnergyStoragePacketUpdate,
    ...
    public static final long ENERGY_USAGE_PER_TICK = ModConfigs.COMMON_ADVANCED_CRUSHER_ENERGY_CONSUMPTION_PER_TICK.getValue();
    ...
    public ScreenHandler createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
        ...
        return new AdvancedCrusherMenu(id, this, inventory, internalInventory, upgradeModuleInventory, this.data);
    }
    ...
    public static void tick(World level, BlockPos blockPos, BlockState state, AdvancedCrusherBlockEntity blockEntity) {
        if(level.isClient())
            return;

        if(!blockEntity.redstoneMode.isActive(state.get(AdvancedCrusherBlock.POWERED)))
            return;
    ...
    private static boolean hasRecipe(AdvancedCrusherBlockEntity blockEntity) {
        ...
        return blockEntity.fluidStorage.parts.get(0).getAmount() >= WATER_CONSUMPTION_PER_RECIPE &&
                ...
                recipe.isPresent() && canInsertItemIntoOutputSlot(blockEntity.internalInventory, recipe.get().value().getResult(level.getRegistryManager()));
    }
