...
public class AlloyFurnaceBlockEntity
    ...
    public AlloyFurnaceBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(
                ModBlockEntities.ALLOY_FURNACE_ENTITY, blockPos, blockState,
    ...
    public static void tick(World level, BlockPos blockPos, BlockState state, AlloyFurnaceBlockEntity blockEntity) {
        ...
        if(blockEntity.hasRecipe()) {
            ...
            if(blockEntity.maxProgress == 0)
                blockEntity.maxProgress = Math.max(1, (int)(recipe.get().getTicks() * RECIPE_DURATION_MULTIPLIER));
    ...
    private Optional<AlloyFurnaceRecipe> getRecipeFor(SimpleInventory inventory) {
        return world.getRecipeManager().getFirstMatch(ModRecipes.ALLOY_FURNACE_TYPE, inventory, world);
    }
    ...
    private boolean hasRecipe() {
        ...
        Optional<AlloyFurnaceRecipe> recipe = getRecipeFor(itemHandler);
    ...
    protected void craftItem(AlloyFurnaceRecipe recipe) {
        ...
        itemHandler.setStack(4, ItemStackUtils.copyWithCount(outputs[0],
                itemHandler.getStack(4).getCount() + outputs[0].getCount()));
        if(!outputs[1].isEmpty())
            itemHandler.setStack(5, ItemStackUtils.copyWithCount(outputs[1],
                    itemHandler.getStack(5).getCount() + outputs[1].getCount()));

        resetProgress();
    }

    private boolean canCraftRecipe(SimpleInventory inventory, AlloyFurnaceRecipe recipe) {
