...
public class InductionSmelterBlockEntity extends SimpleRecipeMachineBlockEntity<AlloyFurnaceRecipe> {
    ...
    protected void craftItem(AlloyFurnaceRecipe recipe) {
        ...
        itemHandler.setStack(3, ItemStackUtils.copyWithCount(outputs[0],
                itemHandler.getStack(3).getCount() + outputs[0].getCount()));
        if(!outputs[1].isEmpty())
            itemHandler.setStack(4, ItemStackUtils.copyWithCount(outputs[1],
                    itemHandler.getStack(4).getCount() + outputs[1].getCount()));
