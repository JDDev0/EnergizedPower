...
public class FiltrationPlantBlockEntity
    ...
    public FiltrationPlantBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(
                ModBlockEntities.FILTRATION_PLANT_ENTITY, blockPos, blockState,

                "filtration_plant", FiltrationPlantMenu::new,

                4,
                ModRecipes.FILTRATION_PLANT_TYPE,
                ModRecipes.FILTRATION_PLANT_SERIALIZER,
    ...
    protected void craftItem(FiltrationPlantRecipe recipe) {
        ...
        if(!outputs[0].isEmpty())
            itemHandler.setStack(2, ItemStackUtils.copyWithCount(outputs[0],
                    itemHandler.getStack(2).getCount() + outputs[0].getCount()));
        if(!outputs[1].isEmpty())
            itemHandler.setStack(3, ItemStackUtils.copyWithCount(outputs[1],
                    itemHandler.getStack(3).getCount() + outputs[1].getCount()));

        resetProgress();
    }

    @Override
    protected boolean canCraftRecipe(SimpleInventory inventory, FiltrationPlantRecipe recipe) {
        ...
        return world != null &&
                ...
                itemHandler.getStack(0).isOf(ModItems.CHARCOAL_FILTER) &&
                itemHandler.getStack(1).isOf(ModItems.CHARCOAL_FILTER) &&
