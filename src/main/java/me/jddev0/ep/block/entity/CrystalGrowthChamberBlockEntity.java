...
public class CrystalGrowthChamberBlockEntity extends SimpleRecipeMachineBlockEntity<CrystalGrowthChamberRecipe> {
    ...
    protected void craftItem(CrystalGrowthChamberRecipe recipe) {
        ...
        ItemStack output = recipe.generateOutput(world.random);

        if(!output.isEmpty())
            itemHandler.setStack(1, ItemStackUtils.copyWithCount(output,
    ...
    @Override
    protected boolean canCraftRecipe(SimpleInventory inventory, CrystalGrowthChamberRecipe recipe) {
        return world != null &&
                InventoryUtils.canInsertItemIntoSlot(inventory, 1, recipe.getMaxOutputCount());
    }
}