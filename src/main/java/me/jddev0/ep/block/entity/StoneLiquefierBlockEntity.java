...
public class StoneLiquefierBlockEntity
        extends SimpleRecipeFluidMachineBlockEntity<SimpleFluidStorage, StoneLiquefierRecipe> {
    ...
    protected void craftItem(StoneLiquefierRecipe recipe) {
        ...
        FluidStack output = new FluidStack(recipe.getOutput().getFluidVariant().getFluid(),
                recipe.getOutput().getFluidVariant().getNbt(), recipe.getOutput().getDropletsAmount());
    ...
    @Override
    protected boolean canCraftRecipe(SimpleInventory inventory, StoneLiquefierRecipe recipe) {
        long fluidAmountInTank = fluidStorage.getFluid().getDropletsAmount();
        long fluidAmountInRecipe = recipe.getOutput().getDropletsAmount();

        return world != null && fluidStorage.getCapacity() - fluidAmountInTank >= fluidAmountInRecipe &&
                (fluidStorage.isEmpty() || (fluidStorage.getResource().isOf(recipe.getOutput().getFluid()) &&
                        fluidStorage.getResource().nbtMatches(recipe.getOutput().getFluidVariant().getNbt())));
    }
}