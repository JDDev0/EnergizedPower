...
public class ChargerBlockEntity
    ...
    public static void tick(World level, BlockPos blockPos, BlockState state, ChargerBlockEntity blockEntity) {
        ...
        if(blockEntity.hasRecipe()) {
            ...
            Optional<ChargerRecipe> recipe = level.getRecipeManager().getFirstMatch(ChargerRecipe.Type.INSTANCE, blockEntity.itemHandler, level);
            if(recipe.isPresent()) {
            ...
            if(blockEntity.energyConsumptionLeft <= 0) {
                recipe.ifPresent(chargerRecipe ->
                        blockEntity.itemHandler.setStack(0, chargerRecipe.getOutput(level.getRegistryManager()).copyWithCount(1)));
    ...
    protected final long getEnergyConsumptionPerTick() {
        ...
        Optional<ChargerRecipe> recipe = world.getRecipeManager().
                getFirstMatch(ChargerRecipe.Type.INSTANCE, this.itemHandler, world);
        if(recipe.isPresent()) {
