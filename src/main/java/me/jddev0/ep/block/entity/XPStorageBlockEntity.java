...
public class XPStorageBlockEntity extends MenuFluidStorageBlockEntity<EnergizedPowerXPFluidStorage> {
    ...
    protected EnergizedPowerXPFluidStorage initFluidStorage() {
        return new EnergizedPowerXPFluidStorage(XPUtils.getTotalXPFromLevel(baseTankCapacity)) {
            @Override
    ...
    public void onInsertExtractXP(int levels, ServerPlayer player) {
        ...
        if(levels > 0) {
            ...
            long insertedAmount = fluidStorage.insertXP(xpTotalToInsert, IFluidHandler.FluidAction.EXECUTE);
            if(insertedAmount > 0) {
            ...
            long extractedAmount = fluidStorage.extractXP(Math.min(Integer.MAX_VALUE, xpTotalToExtract), IFluidHandler.FluidAction.EXECUTE);
