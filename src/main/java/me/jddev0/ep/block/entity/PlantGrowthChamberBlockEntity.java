...
public class PlantGrowthChamberBlockEntity extends SimpleRecipeMachineBlockEntity<PlantGrowthChamberRecipe> {
    ...
    protected void craftItem(PlantGrowthChamberRecipe recipe) {
        ...
        List<ItemStack> itemStacksInsert = new ArrayList<>(Arrays.asList(recipe.generateOutputs(level.random)));
        ...
        for(ItemStack itemStack:itemStacksInsert) {
            ...
            for(int i = 2;i < itemHandler.getSlots();i++) {
                ...
                if(ItemStack.isSameItemSameTags(itemStack, testItemStack)) {
                    ...
                    if(amount > 0) {
                        itemHandler.setStackInSlot(i, ItemStackUtils.copyWithCount(itemHandler.getStackInSlot(i),
