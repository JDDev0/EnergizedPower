    ...
    private void updateRecipe(int index) {
        ...
        if(hasRecipeLoaded[index] && craftingRecipe[index] != null && oldCopyOfRecipe[index] != null) {
            ...
            oldResult = craftingRecipe[index] instanceof SpecialCraftingRecipe ?craftingRecipe[index].
                    craft(oldCopyOfRecipe[index]):
                    craftingRecipe[index].getOutput();
        }
        ...
        for(ItemStack itemStack:itemStacksInsert) {
            for(int i = 0;i < itemHandler.size();i++) {
                ...
                if(ItemStack.areItemsEqual(itemStack, testItemStack) && ItemStack.canCombine(itemStack, testItemStack)) {
                    ...
                    if(amount > 0) {
                        itemHandler.setStack(i, ItemStackUtils.copyWithCount(itemHandler.getStack(i), testItemStack.getCount() + amount));
    ...
    private boolean isOutputOrCraftingRemainderOfInput(ItemStack itemStack) {
        for(int i = 0;i < 3;i++) {
            ...
            ItemStack resultItemStack = craftingRecipe[i] instanceof SpecialCraftingRecipe?
                    craftingRecipe[i].craft(copyOfPatternSlots):
                    craftingRecipe[i].getOutput();
