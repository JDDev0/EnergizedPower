...
public class AdvancedAutoCrafterBlockEntity
    ...
    private void updateRecipe(int index) {
        ...
        if(hasRecipeLoaded[index] && craftingRecipe[index] != null && oldCopyOfRecipe[index] != null) {
            ...
            oldResult = craftingRecipe[index] instanceof SpecialCraftingRecipe ?craftingRecipe[index].
                    craft(oldCopyOfRecipe[index], world.getRegistryManager()):
                    craftingRecipe[index].getOutput(world.getRegistryManager());
        }
        ...
        Optional<Pair<Identifier, CraftingRecipe>> recipe = getRecipeFor(copyOfPatternSlots, world, recipeIdForSetRecipe[index]);
        if(recipe.isPresent()) {
            ...
            ItemStack resultItemStack = craftingRecipe[index] instanceof SpecialCraftingRecipe?craftingRecipe[index].
    ...
    private void craftItem(int index, CraftingInventory copyOfPatternSlots) {
        ...
        for(ItemStack remainingItem:craftingRecipe[index].getRemainder(copyOfPatternSlots))
    ...
    private boolean isOutputOrCraftingRemainderOfInput(ItemStack itemStack) {
        for(int i = 0;i < 3;i++) {
            ...
            ItemStack resultItemStack = craftingRecipe[i] instanceof SpecialCraftingRecipe?
                    craftingRecipe[i].craft(copyOfPatternSlots, world.getRegistryManager()):
                    craftingRecipe[i].getOutput(world.getRegistryManager());

            if(ItemStack.areItemsEqual(itemStack, resultItemStack) && ItemStack.canCombine(itemStack, resultItemStack))
                return true;

            for(ItemStack remainingItem:craftingRecipe[i].getRemainder(copyOfPatternSlots))
    ...
    private List<CraftingRecipe> getRecipesFor(CraftingInventory patternSlots, World level) {
        return level.getRecipeManager().listAllOfType(RecipeType.CRAFTING).
                ...
                sorted(Comparator.comparing(recipe -> recipe.getOutput(level.getRegistryManager()).getTranslationKey())).
                toList();
    }
