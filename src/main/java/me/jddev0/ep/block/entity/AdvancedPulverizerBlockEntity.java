...
public class AdvancedPulverizerBlockEntity
        extends SimpleRecipeFluidMachineBlockEntity<CombinedStorage<FluidVariant, SimpleFluidStorage>, PulverizerRecipe> {
    ...
    protected void craftItem(PulverizerRecipe recipe) {
        ...
        ItemStack[] outputs = recipe.generateOutputs(world.random, true);
        ...
        itemHandler.setStack(1, ItemStackUtils.copyWithCount(outputs[0],
                itemHandler.getStack(1).getCount() + outputs[0].getCount()));
        if(!outputs[1].isEmpty())
            itemHandler.setStack(2, ItemStackUtils.copyWithCount(outputs[1],
                    itemHandler.getStack(2).getCount() + outputs[1].getCount()));
