    ...
    protected SimpleFluidStorage initFluidStorage() {
        return new SimpleFluidStorage(baseTankCapacity) {
            ...
            private boolean isFluidValid(FluidVariant variant) {
                ...
                Collection<RecipeHolder<FluidFreezerRecipe>> recipes = RecipeUtils.getAllRecipesFor(serverWorld, FluidFreezerRecipe.Type.INSTANCE);

                return recipes.stream().map(RecipeHolder::value).map(FluidFreezerRecipe::getInput).
    ...
    protected boolean canCraftRecipe(SimpleContainer inventory, RecipeHolder<FluidFreezerRecipe> recipe) {
        return level != null &&
                (fluidStorage.getResource().isOf(recipe.value().getInput().getFluid()) &&
                        fluidStorage.getResource().componentsMatch(recipe.value().getInput().getFluidVariant().getComponentsPatch())) &&
