...
public class FluidTransposerBlockEntity
    ...
    protected SimpleFluidStorage initFluidStorage() {
        return new SimpleFluidStorage(baseTankCapacity) {
            ...
            private boolean isFluidValid(FluidVariant variant) {
                ...
                return world.getRecipeManager().listAllOfType(recipeType).stream().
    ...
    @Override
    protected Optional<FluidTransposerRecipe> getRecipeFor(SimpleInventory inventory) {
        return world.getRecipeManager().listAllOfType(recipeType).
                ...
                filter(recipe -> (mode == Mode.EMPTYING && fluidStorage.isEmpty()) ||
                        (recipe.getFluid().getFluidVariant().isOf(fluidStorage.getFluid().getFluid()) &&
                                recipe.getFluid().getFluidVariant().nbtMatches(fluidStorage.getFluid().getFluidVariant().getNbt()))).
                findFirst();
    }
    ...
    protected void craftItem(FluidTransposerRecipe recipe) {
        ...
        NbtCompound nbt = recipe.getFluid().getFluidVariant().getNbt();
        FluidStack fluid = new FluidStack(recipe.getFluid().getFluidVariant().getFluid(),
                nbt == null?null:nbt.copy(), recipe.getFluid().getDropletsAmount());
    ...
    @Override
    protected boolean canCraftRecipe(SimpleInventory inventory, FluidTransposerRecipe recipe) {
        long fluidAmountInTank = fluidStorage.getFluid().getDropletsAmount();
        long fluidAmountInRecipe = recipe.getFluid().getDropletsAmount();

        return world != null &&
                (mode == Mode.EMPTYING?fluidStorage.getCapacity() - fluidAmountInTank:fluidAmountInTank) >= fluidAmountInRecipe &&
                (mode != Mode.EMPTYING || fluidStorage.isEmpty() || (fluidStorage.getResource().isOf(recipe.getFluid().getFluid()) &&
                        fluidStorage.getResource().nbtMatches(recipe.getFluid().getFluidVariant().getNbt()))) &&
