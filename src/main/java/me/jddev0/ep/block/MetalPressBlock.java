...
public class MetalPressBlock extends BlockWithEntity {
    public static final MapCodec<MetalPressBlock> CODEC = createCodec(MetalPressBlock::new);

    public static final BooleanProperty POWERED = Properties.POWERED;

    public MetalPressBlock(AbstractBlock.Settings props) {
        super(props);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World level, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, EPBlockEntities.METAL_PRESS_ENTITY, MetalPressBlockEntity::tick);
    }
    ...
    public static class Item extends BlockItem {
        ...
        public void appendTooltip(ItemStack stack, TooltipContext context, TooltipDisplayComponent displayComponent, Consumer<Text> tooltip, TooltipType type) {
            if(Screen.hasShiftDown()) {
                tooltip.accept(Text.translatable("tooltip.energizedpower.metal_press.txt.shift.1").
