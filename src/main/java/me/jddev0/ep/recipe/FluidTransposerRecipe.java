...
public class FluidTransposerRecipe implements Recipe<RecipeInput> {
     ...
     */
    public FluidTransposerRecipe(ItemStack output, Ingredient input, FluidIngredientWithAmount fluid) {
        this(FluidTransposerBlockEntity.Mode.FILLING, output, input, Either.right(fluid));
    }

    public FluidTransposerRecipe(FluidTransposerBlockEntity.Mode mode, ItemStack output, Ingredient input, Either<FluidStack, FluidIngredientWithAmount> fluid) {
        this.mode = mode;
        this.output = output;
        this.input = input;
        this.fluid = fluid;
    ...
    public Either<FluidStack, FluidIngredientWithAmount> getFluid() {
        return fluid;
    }
    ...
    public static final class Serializer implements RecipeSerializer<FluidTransposerRecipe> {
        ...
        private final MapCodec<FluidTransposerRecipe> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
            return instance.group(FluidTransposerBlockEntity.Mode.CODEC.fieldOf("mode").forGetter((recipe) -> {
                ...
                return recipe.input;
            }), Codec.either(FluidStack.CODEC_MILLIBUCKETS, FluidIngredientWithAmount.CODEC).fieldOf("fluid").forGetter((recipe) -> {
                return recipe.fluid;
        ...
        private static FluidTransposerRecipe read(RegistryFriendlyByteBuf buffer) {
            ...
            Either<FluidStack, FluidIngredientWithAmount> fluid;
            if(buffer.readBoolean()) {
                fluid = Either.left(FluidStack.STREAM_CODEC.decode(buffer));
            }else {
