...
public final class FluidIngredient {
    ...
    private static final StreamCodec<ByteBuf, TagKey<Fluid>> FLUID_TAG_KEY_STREAM_CODEC = StreamCodecFix.tagKeyStreamCodec(BuiltInRegistries.FLUID.key());
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, FluidIngredient> STREAM_CODEC = StreamCodec.of((buffer, fluidIngredient) -> {
        ...
        if(buffer.readBoolean()) {
            ...
            for(int i = 0;i < fluidCount;i++)
                fluids.add(BuiltInRegistries.FLUID.get(buffer.readResourceLocation()));
    ...
    public boolean matches(FluidStack fluid) {
        return matches(fluid.getFluidVariant());
    }

    public boolean matches(FluidVariant fluid) {
        return this.fluid.map(
                fluids -> fluids.stream().anyMatch(f -> fluid.isOf(f)),
                fluidTag -> fluid.getRegistryEntry().is(fluidTag)
        );
    }
